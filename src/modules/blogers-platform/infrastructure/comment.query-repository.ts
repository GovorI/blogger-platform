...
export class CommentsQueryRepository {
  ...
  async getByIdOrNotFoundFail(
  ...
  ): Promise<CommentDocument & { myStatus: string }> {
    ...
    if (currentUserId) {
      try {
        const userLike = await this.likesRepository.getLikeByCommentIdAndUserId(
          id,
          currentUserId,
        );
        if (userLike) {
    ...
    // comment.myStatus = myStatus;
    // return comment as unknown as CommentDocument & { myStatus: string };
    return { ...comment.toObject(), myStatus } as CommentDocument & {
      myStatus: string;
    };
  }
  ...
  async getAllCommentsForPost(
    ...
    for (const comment of comments) {
      ...
      if (currentUserId) {
        try {
          const userLike =
            await this.likesRepository.getLikeByCommentIdAndUserId(
      ...
      const commentViewDto = CommentViewDto.mapToView(
        comment,
        currentUserId,
        myStatus,
      );
