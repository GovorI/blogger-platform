...
export class LikesRepository {
  ...
  async getLikeByPostIdAndUserId(
    postId: string,
    userId: string,
  ): Promise<LikeStatusPostDocument | null> {
    return this.LikeStatusPostModel.findOne({ postId, userId });
  }

  async save(
    likeStatus: LikeStatusPostDocument | LikeStatusCommentDocument,
  ): Promise<void> {
    await likeStatus.save();
  }

  async getNewestLikesForPost(postId: string, limit: number = 3) {
    return await this.LikeStatusPostModel.find({ postId, status: 'Like' })
      .sort({ createdAt: -1 })
      .limit(limit)
  ...
  async getLikeByCommentIdAndUserIdOrFail(commentId: string, userId: string) {
    const like = await this.LikeStatusCommentModel.findOne({
      commentId,
      userId,
    });
    if (!like) {
